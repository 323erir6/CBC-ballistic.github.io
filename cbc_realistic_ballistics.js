...
{id:"military_small_single",name:"Military Supplement Small Single Cannon",caliber:.060,velocityModel:"military_dual",massMultiplier:1.20},
{id:"military_small_dual",name:"Military Supplement Small Dual Cannon",caliber:.060,velocityModel:"military_dual",massMultiplier:1.20},
{id:"military_small_medium_single",name:"Military Supplement Medium Single Cannon",caliber:.080,velocityModel:"military_dual",massMultiplier:1.45},
{id:"military_small_medium_dual",name:"Military Supplement Medium Dual Cannon",caliber:.080,velocityModel:"military_dual",massMultiplier:1.45},
{id:"military_large_single",name:"Military Supplement Large Cannon",caliber:.125,velocityModel:"military_dual",massMultiplier:1.89},
...
function rocket(id){let v=id.toLowerCase();return /^cbc_at:(?:(?:ap|flak|he|hei)_rocket|medium_.*_rocket)$/.test(v)||/^cbcatfix:big_.*_rocket$/.test(v)}
function cd(id){let v=id.toLowerCase();if(rocket(v))return .12;if(v.includes("traffic_cone"))return .9;if(v.includes("mortar")||v.includes("grapeshot"))return .47;if(v.includes("mediumshell")){if(v.includes("apfsds")||v.includes("apds"))return .18;if(v.includes("aphe")||v.includes(":ap_"))return .22;return .25}if(v==="createbigcannons:ap_shot"||v==="createbigcannons:shot"||/(?:^|_)ap_shot$/.test(v))return .28;if(v.includes("ap_shell")||v.includes("apshell"))return .22;if(v.includes("autocannon")||v.includes("machine_gun"))return .2;if(v.includes("shell"))return .25;return .3}
const EXTRA_PROJECTILES=[["cbcatfix:big_ap_rocket",4],["cbcatfix:big_he_rocket",4],["cbcatfix:big_heat_rocket",4]];
const PROJECTILES=RAW.concat(EXTRA_PROJECTILES).map(([id,referenceMass])=>({id,referenceMass,diameter:diam(id),cd:cd(id),lengthCalibers:rocket(id)?12:3})).sort((a,b)=>a.id.localeCompare(b.id)),PM=new Map(PROJECTILES.map(x=>[x.id,x]));
...
function isProjectileCompatible(c,id){if(!load(id))return false;switch(c){case"manual":return true;case"cbc_big_cannon":return big(id);case"cbc_drop_mortar":return /^createbigcannons:(?:drop_mortar_shell|mortar_stone)$/.test(id);case"cbc_autocannon":case"modern_rotary_cannon":return AUTO.test(id);case"military_small_single":case"military_small_dual":case"military_small_medium_single":case"military_small_medium_dual":return /^cbcmoreshells:normal_/.test(id);case"military_large_single":return /^cbcmoreshells:(?:normal_|extended_)/.test(id);case"military_torpedo_tube":case"modern_torpedo_tube":return TORP.test(id);case"modern_medium_cannon":return /^cbcmodernwarfare:[a-z0-9_]*mediumshell$/.test(id);case"cbc_at_heavy_autocannon":return HAT.test(id);case"cbc_at_twin_autocannon":case"cbc_at_vertical_twin_autocannon":return LAT.test(id);case"cbc_at_rocket_pod":return /^cbc_at:(?:ap|flak|he|hei)_rocket$/.test(id);case"cbc_at_medium_rocket_rail":return /^cbc_at:medium_(?:ap|he|hef|heat)_rocket$/.test(id);case"cbcatfix_big_rocket_rail":return /^cbcatfix:big_(?:ap|he|heat)_rocket$/.test(id);default:return false}}
function rocketMotorProfile(c,id){if(c==="cbc_at_rocket_pod"&&/^cbc_at:(?:ap|flak|he|hei)_rocket$/.test(id))return{step:.00735,max:.735,fuelTicks:54};if(c==="cbc_at_medium_rocket_rail"&&/^cbc_at:medium_.*_rocket$/.test(id))return{step:.016425,max:1.231875,fuelTicks:54};if(c==="cbcatfix_big_rocket_rail"&&/^cbcatfix:big_.*_rocket$/.test(id))return{step:.03065,max:.27585,fuelTicks:127};return null}
function projectileDefaults(id){return PM.get(id)||{id,referenceMass:2,diameter:diam(id||""),cd:cd(id||""),lengthCalibers:rocket(id||"")?12:3}}
