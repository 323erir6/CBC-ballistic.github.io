...
{id:"cbc_drop_mortar",name:"CBC Drop Mortar",caliber:null},
{id:"cbc_autocannon",name:"CBC Autocannon (steel)",caliber:.02,velocityModel:"cbc_autocannon"},
{id:"military_small_single",name:"Military Supplement Small Single Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_small_dual",name:"Military Supplement Small Dual Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_small_medium_single",name:"Military Supplement Medium Single Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_small_medium_dual",name:"Military Supplement Medium Dual Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_large_single",name:"Military Supplement Large Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_large_dual",name:"Military Supplement Giant Cannon",caliber:null,velocityModel:"military_dual"},
{id:"military_projectile_rack",name:"Military Supplement Projectile Rack",caliber:null,velocityModel:"military_rack"},
{id:"military_torpedo_tube",name:"Military Supplement Torpedo Tube",caliber:null,velocityModel:"military_torpedo"},
...
{id:"modern_gun_launcher",name:"CBC Modern Warfare Gun Launcher",caliber:null},
{id:"modern_torpedo_tube",name:"CBC Modern Warfare Torpedo Tube",caliber:null,velocityModel:"military_torpedo"},
...
{id:"cbc_at_rocket_pod",name:"CBC AT Rocket Pod (steel)",caliber:null,velocityModel:"cbc_at_rocket"},
{id:"cbc_at_medium_rocket_rail",name:"CBC AT Medium Rocket Rail (steel)",caliber:null,velocityModel:"cbc_at_rocket"},
{id:"cbcatfix_big_rocket_rail",name:"CBC AT Fix Big Rocket Rail (steel)",caliber:null,velocityModel:"cbc_at_rocket"},
{id:"arcon_cannon",name:"ARCON Cannon",caliber:null,velocityModel:"arcon"}];
const MD={"cbcmoreshells:normal_ap_shot":8,"cbcmoreshells:normal_ap_shell":8,"cbcmoreshells:normal_apbc_shell":8,"cbcmoreshells:normal_sap_shell":8,"cbcmoreshells:normal_he_shell":8,"cbcmoreshells:normal_incendiary_he_shell":8,"cbcmoreshells:normal_antiair_he_shell":8,"cbcmoreshells:extended_ap_shot":10,"cbcmoreshells:extended_antiair_he_shell":12};
...
function fixedMuzzleVelocity(c,p,n){switch(CANNONS.find(x=>x.id===c)?.velocityModel){case"military_dual":return(MD[p]??4)*TPS;case"military_rack":return(MR[p]??.5)*TPS;case"military_torpedo":return(MT[p]??1)*TPS;case"cbc_autocannon":return bv(n,3,1.5,4,1);case"modern_medium":return bv(n,3,1,11,1);case"modern_rotary":return bv(n,3,1.5,3,1);case"cbc_at_rocket":return bv(n,3,1.5,4,.5);case"arcon":return 500;default:return null}}
...
function isProjectileCompatible(c,id){if(!load(id))return false;switch(c){case"manual":return true;case"cbc_big_cannon":return big(id);case"cbc_drop_mortar":return /^createbigcannons:(?:drop_mortar_shell|mortar_stone)$/.test(id);case"cbc_autocannon":case"modern_rotary_cannon":return AUTO.test(id);case"military_small_single":case"military_small_dual":case"military_small_medium_single":case"military_small_medium_dual":case"military_large_single":case"military_large_dual":return DUAL.test(id);case"military_projectile_rack":return RACK.test(id);case"military_torpedo_tube":case"modern_torpedo_tube":return TORP.test(id);case"modern_medium_cannon":return /^cbcmodernwarfare:[a-z0-9_]*mediumshell$/.test(id);case"modern_gun_launcher":return id==="cbcmodernwarfare:munitions_contraption";case"cbc_at_heavy_autocannon":return HAT.test(id);case"cbc_at_twin_autocannon":case"cbc_at_vertical_twin_autocannon":return LAT.test(id);case"cbc_at_rocket_pod":return /^cbc_at:(?:ap|flak|he|hei)_rocket$/.test(id);case"cbc_at_medium_rocket_rail":return /^cbc_at:medium_(?:ap|he|hef|heat)_rocket$/.test(id);case"cbcatfix_big_rocket_rail":return /^cbc_at:(?:medium_)?(?:ap|flak|he|hei|hef|heat)_rocket$/.test(id);case"arcon_cannon":return id==="arcon:bops_projectile";default:return false}}
